...
DSP_INLINE 
vi SquaredNorm (const vcs&a ) {
	return pairwise_muladd ((vs&)a,(vs&)a);
}
...
//   2. elements of the resulting product has double-length field compared to source elements
DSP_INLINE void conj_mul(vi& re, vi& im, const vcs& a, const vcs& b)
{
    vcs vs1 = permutate_low<1, 0, 3, 2>(b);
...
//
// cmul_ex - perform multiple of two complex vectors and store the real
// and image component of the result to two real vectors
//
DSP_INLINE 
void mul_ex (vi& re, vi& im, const vcs& a, const vcs& b)
{
	vcs vs1 = conj0 (b);
	// vs2 = (d0, c0, d1, c1, d2, c2, d3, c2)
	vcs vs2 = permutate_high<1, 0, 3, 2>(permutate_low<1, 0, 3, 2>(b));
...
template <typename T> 
void DSP_INLINE load (T& r, void* p) { r = _mm_loadu_si128 ((__m128i *)p); }
...
#define DEFINE_OP_INSET(OP, T, INSTRINSIC, ET)  \
template<int ndx>								\
...
DEFINE_OP_INSET(insert, vcs, _mm_insert_epi32, UINT) 
DEFINE_OP_INSET(insert, vs,  _mm_insert_epi16, short)
