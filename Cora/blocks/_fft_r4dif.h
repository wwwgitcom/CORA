...
#include "_fft_lut_bitreversal.h"
#include "_fft_lut_twiddle.h"
...
DSP_INLINE1 void FFTSSE(vcs* pInput)
{
    ...
    for (int n = 0; n < nArray/4; n++, pi++)
    {
        ...
        vcs a_c = saturation_sub(a, c);
        vcs b_d = saturation_sub(b, d);
...
FFTSSEEx<8> (vcs* pInput)
{
    ...
    xmm0 = saturation_add(xmm0, xmm1);					// xmm0 = a + b, for 4-point FFT
    ...
    xmm3 = sub(xmm3, xmm5);                             // xmm0 and xmm3 store 4-point data
    ...
    xmm0 = xor(xmm0, xmm3);                             // xmm0 = -Q3 -I3 Q2 I2 -Q1 -I1 Q0 I0
    xmm0 = sub(xmm0, xmm3);
    xmm0 = saturation_add(xmm0, xmm2);                  // 4-FFT Over
