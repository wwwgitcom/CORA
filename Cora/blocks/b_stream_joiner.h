...
DEFINE_BLOCK(b_stream_joiner_1_2v1, 2, 1)
{
  ...
  BLOCK_WORK
  {
    trace();
    auto n = ninput(0);
    if (n < *count_per_stream) return false;
    ...
    for (int iIn = 0; iIn < *count_per_stream; iIn++, iOut += 2)
    {
      op[iOut]     = ip1[iIn];
      op[iOut + 1] = ip2[iIn];
    }
...
DEFINE_BLOCK(b_stream_joiner_2_2v1, 2, 1)
{
  ...
  BLOCK_WORK
  {
    ...
    for (int iIn = 0; iIn < *count_per_stream; iIn += 2, iOut += 4)
    {
      op[iOut + 0] = ip1[iIn + 0];
      op[iOut + 1] = ip1[iIn + 1];
      op[iOut + 2] = ip2[iIn + 0];
...
DEFINE_BLOCK(b_stream_joiner_3_2v1, 2, 1)
{
  ...
  BLOCK_WORK
  {
    ...
    for (int iIn = 0; iIn < *count_per_stream; iIn += 3, iOut += 6)
    {
      op[iOut + 0] = ip1[iIn + 0];
      op[iOut + 1] = ip1[iIn + 1];
      op[iOut + 2] = ip1[iIn + 2];
